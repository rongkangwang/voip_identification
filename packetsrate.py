...
def saveitd2xlsx():
    ...
    for sheet in tables:
        ...
        for i in range(len(packets_init)-1):
            src = packets_init[i][1][26:30]
            src = str(ord(src[0]))+"."+str(ord(src[1]))+"."+str(ord(src[2]))+"."+str(ord(src[3]))
            des = packets_init[i][1][30:34]
            des = str(ord(des[0]))+"."+str(ord(des[1]))+"."+str(ord(des[2]))+"."+str(ord(des[3]))
            if(srcanddesmap.has_key(src+" "+des) or srcanddesmap.has_key(des+" "+src)):
                if(srcanddesmap.has_key(src+" "+des)):
                    srcanddesmap[src+" "+des] += 1
                if(srcanddesmap.has_key(des+" "+src)):
                    srcanddesmap[des+" "+src] += 1
            else:
                srcanddesmap[src+" "+des] = 1
        for key,value in  srcanddesmap.items():
            if(value>count_srcanddes):
                count_srcanddes = value
                max_srcanddes = key
        print max_srcanddes
        ...
        for i in range(len(packets_init)-1):
            ...
            des = packets_init[i][1][30:34]
            des = str(ord(des[0]))+"."+str(ord(des[1]))+"."+str(ord(des[2]))+"."+str(ord(des[3]))
            if(src+" "+des==max_srcanddes):
                srcnum+=1
                srcdesnum+=1
            if(des+" "+src==max_srcanddes):
                desnum+=1
                srcdesnum+=1
            if(srcdesnum==5):
                print str(srcnum)+" "+str(desnum)
                print((srcnum+1.0)/(desnum+1.0))
                sheet.write(tr,0,(srcnum+1.0)/(desnum+1.0))
                tr = tr+1
                srcdesnum = 0
                srcnum = 0
                desnum = 0
    workbook.save("rate.xlsx")
